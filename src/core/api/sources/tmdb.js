...
function parseCountries(movie){
    ...
    let countries = movie && movie.production_countries

    if(!countries) return []

    if(typeof countries == 'string'){
        countries = countries.split(',').map((name)=>name.trim()).filter(Boolean)
    }
    ...
    return countries.map((a)=>{
        ...
        if(a.iso_3166_1 && iso_countries.hasOwnProperty(a.iso_3166_1)){
