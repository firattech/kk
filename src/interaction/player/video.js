 ...
 function url(src, change_quality){
    ...
    if(/\.mpd/.test(src) && typeof dashjs !== 'undefined'){
        ...
        if(typeof Hls !== 'undefined'){
            ...
            // IPTV/canli yayinda native HLS tarayicida sik sik bos player birakiyor.
            // Hls.js varsa bu akista programmatic parser'i zorla kullanalim.
            if(TV.playning() && Hls.isSupported()) use_program = true
            ...
            if(use_program){
                        ...
                        xhr.addEventListener('loadend', function(){
                            try{
                                if(xhr.responseURL && xhr.responseURL !== url){
                                    console.log('Player', 'hls redirect:', url, '->', xhr.responseURL)
                                }
                            }
                            catch(e){}
                        })
                ...
                hls.on(Hls.Events.MANIFEST_LOADED, function(){
                    try{
                        let details = arguments[1] && arguments[1].networkDetails
                        let finalUrl = details && details.responseURL ? details.responseURL : src
                        console.log('Player', 'hls manifest loaded:', finalUrl)
                    }
                    catch(e){}
                    play()
                })
                ...
                hls_parser = new Hls({
                    ...
                    xhrSetup: function(xhr, url) {
                        ...
                        xhr.addEventListener('loadend', function(){
                            try{
                                if(xhr.responseURL && xhr.responseURL !== url){
                                    console.log('Player', 'hls parser redirect:', url, '->', xhr.responseURL)
                                }
                            }
                            catch(e){}
                        })
                ...
                hls_parser.on(Hls.Events.MANIFEST_LOADED, function(){
                    try{
                        let details = arguments[1] && arguments[1].networkDetails
                        let finalUrl = details && details.responseURL ? details.responseURL : src
                        console.log('Player', 'hls parser manifest loaded:', finalUrl)
                    }
                    catch(e){}
...
video && video.addEventListener && video.addEventListener('loadedmetadata', function(){
    try{
        console.log('Player', 'video currentSrc:', video.currentSrc || video.src || '')
    }
    catch(e){}
})
