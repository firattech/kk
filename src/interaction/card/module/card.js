...
export default {
    onInit: function(){
        this.getPosterPath = ()=>{
            let src = ''
            let absolutePoster = /^https?:\/\//i.test((this.data.poster_path || '') + '')
            let absoluteBackdrop = /^https?:\/\//i.test((this.data.backdrop_path || '') + '')
            let absoluteProfile = /^https?:\/\//i.test((this.data.profile_path || '') + '')
                
            if(this.params.style.name == 'wide' && this.data.backdrop_path) src = absoluteBackdrop ? this.data.backdrop_path : TMDB.img(this.data.backdrop_path, 'w780')
            else if(this.params.style.name == 'collection' && this.data.backdrop_path) src = absoluteBackdrop ? this.data.backdrop_path : TMDB.img(this.data.backdrop_path, 'w500')
            else if(this.data.poster_path)  src = absolutePoster ? this.data.poster_path : TMDB.img(this.data.poster_path)
            else if(this.data.profile_path) src = absoluteProfile ? this.data.profile_path : TMDB.img(this.data.profile_path)
