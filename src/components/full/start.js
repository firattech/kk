...
class Start extends Emit {
    constructor(data) {
        ...
        let poster = this.card.poster_path || ''
        let poster_img = /^https?:\/\//i.test(poster) ? poster : (poster ? Api.img(poster, Storage.field('poster_size')).replace(/\/w\d{1,4}/,'/w500') : '')

        Arrays.extend(this.card,{
            title: this.card.name,
            original_title: this.card.original_name,
            runtime: 0,
            img: this.card.img || poster_img || './img/img_broken.svg'
        })
        ...
        this.use(Options)
        this.use(Subscribed)
        if(this.card.source !== 'makra') this.use(Trailers)
        this.use(Poster)
        if(this.card.source !== 'makra') this.use(Translations)
        this.use(Bookmarks)
        if(this.card.source !== 'makra') this.use(Reactions)
        this.use(Buttons)
    ...
    create(){
        let title       = this.card.title || this.card.name || this.card.original_title || this.card.original_name || '...'
        let original    = this.card.original_title || this.card.original_name || title
        let genres      = (Array.isArray(this.card.genres) ? this.card.genres : []).slice(0,3).map(a => Utils.capitalizeFirstLetter((a && a.name) || a || '')).filter(Boolean).join(', ')
        ...
        if(!this.card.tagline && title.length > 25) this.html.find('.full-start-new__title').addClass('twolines')
